...
void HashTable::PrintAll() {
    ...
    for (Node node : nodes) {
        if (node.key != UINT_MAX) {
            ...
            Node* nextNode = node.next;
            while (nextNode != nullptr) {
                cout << "    " << nextNode->key << ": " << nextNode->bid.bidId << " | "
                    << nextNode->bid.title << " | " << nextNode->bid.amount << " | "
                    << nextNode->bid.fund << endl;
                nextNode = nextNode->next;
            }
        }
    }
...
Bid HashTable::Search(string bidId) {
    ...
    */
    ...
    // Traverse the linked list for the key to find the bid
    while (node->next != nullptr) {
        node = node->next;
        if (node->key != UINT_MAX && node->bid.bidId == bidId) {
            return node->bid;
...
void loadBids(string csvPath, HashTable* hashTable) {
    ...
    for (auto const& c : header) {
        //cout << c << " | ";                                           turning off for now as it is not seen in example output
    }
    //cout << "" << endl;
    try {
